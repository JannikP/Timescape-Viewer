...
use crate::state::{Run, Source, Timeline, Track};
    ...
    R: Read + Seek,
{
    ...
    let track = Track {
        signals: signals
            .signals
            .iter()
            .map(|s| Signal::from(s).reference_counted())
            .collect(),
        ...
        values: signals
            ...
            .map(|s| {
                samples
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "Signal ID '{}' not found in 'Samples' section.",
                            s.signal_id
                        )
                    })
            })
    ...
    let run = Run::with_single_track(track).with_title(general.record_name);
